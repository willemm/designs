...
NeoPixelBus<NeoRgbwFeature, Neo800KbpsMethod> strip(PixelCount, 0);
...
void setup()
{
    ...
    server.on("/build/bundle.css", send_css);
    server.on("/build/bundle.js", send_js);
    */
...
/*
void send_index() {
  File file = SPIFFS.open(F("/index.html.gz"), "r");
  if (server.streamFile(file, F("text/html")) != file.size()) {
    // ERROR
  }
  file.close();
}
...
void send_css() {
  File file = SPIFFS.open(F("/bundle.css.gz"), "r");
  if (server.streamFile(file, F("test/css")) != file.size()) {
    // ERROR
  }
  file.close();
}
...
void send_js() {
  File file = SPIFFS.open(F("/bundle.js.gz"), "r");
  if (server.streamFile(file, F("application/javascript")) != file.size()) {
    // ERROR
  }
  file.close();
}
